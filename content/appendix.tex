...
As shown in Figure~\ref{fig:throughput_combined}, the independent parallelization strategy achieves comparable throughput to \gls{remd} at lower parallel counts, while the \gls{remd} setup plateaus earlier due to the overhead of coordinating replica exchanges.

\begin{figure}[htbp]
    \centering
    % TODO: replace with combined throughput figure
    \fbox{\parbox{0.65\linewidth}{\centering\vspace{2cm}PLACEHOLDER: Combined throughput plot showing independent parallelization vs.\ \gls{remd} (reform) as a function of the number of parallel systems on a single NVIDIA A100 GPU.\vspace{2cm}}}
